...
@Stateless
public class FileMedicoDAOImpl extends AbstractJtaStatelessCRUDServices<Long, FileMedico> implements FileMedicoDAOLocal {
	...
	@Override
	public List<FileMedico> buscar(FileMedicoDTO objeto) throws Exception {
		  List<FileMedico> lista= new ArrayList<FileMedico>();
		  StringBuilder sbQuery = new StringBuilder();
		  sbQuery.append("SELECT  pr.id, ");
		  sbQuery.append("pr.historiaMedica, ");
		  sbQuery.append("pr.numero,pr.nombre, ");
		  sbQuery.append("pr.observacion, ");
		  sbQuery.append("pr.auditoria ");
		  sbQuery.append("FROM  FileMedico pr ");
		  ...
		  if(listaResultado!=null &&  listaResultado.size()>0){
		      for (Object[] valor : listaResultado) {
		    	  FileMedico objetoJPA=new FileMedico();
		    	  objetoJPA.setId((Long)valor[0]);
		    	  objetoJPA.setHistoriaMedica((HistoriaMedica)valor[1]);
		    	  objetoJPA.setNumero((String)valor[2]);
		    	  objetoJPA.setNombre((String)valor[3]);
		    	  objetoJPA.setObservacion((String)valor[4]);
	...
	@Override
	public FileMedico obtener(Long id) throws Exception {
		FileMedico objetoJPA=null;
		try {
			objetoJPA= em.find(FileMedico.class, id);
			em.flush();
		} catch (Exception e) {
	...
	@Override
	public FileMedico insertar(FileMedico objeto) throws Exception {
		try {
			 em.persist(objeto);
		     em.flush();
		} catch (Exception e) {
	...
	@Override
	public Boolean actualizar(FileMedico objeto) throws Exception {
		Boolean isRealizado=null;
		try {
			
			StringBuilder sbQuery = new StringBuilder();
			  sbQuery.append("UPDATE  FileMedico vg ");
			  sbQuery.append("SET ");
	...
	public Boolean eliminarFisica(Long id) throws Exception {
		Boolean isRealizado=null;
		try {
			FileMedico objeto=new FileMedico();
			objeto.setId(id);
			em.remove(objeto);
			isRealizado=true;
		} catch (Exception e) {
